...
class DDoSDetector:
    ...
    def __init__(self, 
        ...
        self.suspicious_flow_counter = 0
    ...
    def run(self):
        ...
        try:
            for message in self.consumer:
                ...
                # Log suspicious flows (Rate limited)
                if prediction.prediction != 'BENIGN':
                    self.suspicious_flow_counter += 1
                    if self.suspicious_flow_counter % 100 == 0:
                        logger.info(
                            f"Suspicious flow (sampled 1/100): {prediction.src_ip} -> {prediction.dst_ip} | "
...
def main():
    ...
    """
    # Configuration
    MODEL_DIR = os.getenv('MODEL_DIR', '/models/final_model')
